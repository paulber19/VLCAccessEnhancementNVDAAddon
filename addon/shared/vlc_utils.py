...
def getTimeList(timeString):
	""" convert time in the form hh:mm:ss
	to a string list of three elements: hours, minutes, seconds """
...
class MessageBox(wx.Dialog):
	...
	def doGui(self):
		...
		textControl = wx.TextCtrl(
			self, -1, style=wx.TE_MULTILINE | wx.TE_READONLY, size=(530, 490))
		textControl.SetValue(self.helpMessage)
		textControl.SetFocus()
		taskSizer.Add(textControl, wx.ALL | wx.EXPAND, wx.ALIGN_CENTRE, 5)
		mainSizer.Add(
			taskSizer,
			flag=wx.LEFT | wx.RIGHT | wx.BOTTOM | wx.ALIGN_CENTRE,
			border=5)
		mainSizer.Fit(self)
