...
class GoToTimeDialog(wx.Dialog):
	...
	def doGui(self):
		hoursList, minutesList, secondesList = self.initMaxs(
			self.curTime[0],
			self.curTime[1],
			self.curTime[2])
		...
		self.secondesCB = wx.ComboBox(
			self,
			-1,
			self.curTime[2],
			size=wx.DefaultSize,
			choices=secondesList[:],
			style=wx.CB_DROPDOWN,
			name="secondesNumber")
	...
	def onComboBox(self, evt):
		hoursList, minutesList, secondesList = self.initMaxs(
			self.hoursCB.GetValue(),
			self.minutesCB.GetValue(),
			self.secondesCB.GetValue())
		hour = self.hoursCB.GetStringSelection()
		minute = self.minutesCB.GetStringSelection()
		seconde = self.secondesCB.GetStringSelection()
	...
	def onGoToButton(self, event):
		if self.timer:
			...
			self.timer = None
		self.jumpTime = [
			self.hoursCB.GetValue(), self.minutesCB.GetValue(),
			self.secondesCB.GetValue()]
