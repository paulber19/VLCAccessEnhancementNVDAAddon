...
SpecialAlphanumeric = {
	"en": printable[:62],
	# "es": printable[:62]+u"·ÈÌÛ˙‡ËÏÚ˘‰ÎÔˆ¸‚ÍÓÙ˚ÒÁ¡…Õ”⁄¿»Ã“ŸƒÀœ÷‹¬ Œ‘€—«"
}
...
class QTEditableText(EditableTextWithAutoSelectDetection):
	...
	def _get_name(self):
		name = super(QTEditableText, self)._get_name()
		if name is not None:
			return name
		try:
	...
	def script_selectNextCh(self, gesture):
		...
		sizeBefore = self.startSelection-self.fakeCaret\
			if self.startSelection > self.fakeCaret\
			else self.fakeCaret-self.startSelection
		gesture.send()
		self.nextCh(1)
		sizeAfter = self.startSelection-self.fakeCaret\
			if self.startSelection > self.fakeCaret\
			else self.fakeCaret-self.startSelection
		if sizeAfter > sizeBefore:
			speech.speakText(_("selected"))
		else:
			if self.value and self.fakeCaret < len(self.value):
	...
	def removeSelection(self, value=""):
		if self.startSelection < 0\
			or self.fakeCaret == self.startSelection or not value:
			return
		if self.startSelection > self.fakeCaret:
		...
		if self.value:
			...
			if self.value[self.fakeCaret] in punctuation:
				...
				try:
					...
					if selection:
						speech.speakText("%s%s" % (
							self.value[oldCaret:self.fakeCaret-1], self.value[self.fakeCaret-1]))
					return
				try:
					while self.value[self.fakeCaret] == " ":
						self.fakeCaret = self.fakeCaret+1
	...
	script_reportCurrentSelection.__doc__ = commands.script_reportCurrentSelection.__doc__  # noqa:E501
...
class VLCQTEditableText(QTEditableText):
	def initOverlayClass(self):
		...
		if self.language not in SpecialAlphanumeric:
			...
			# in your local alphabet, for example the Ò in Spanish.
			self.alphanumeric = printable[:62]+_("SpecialAlphanumericCharactersInYourLanguage")  # noqa:E501
